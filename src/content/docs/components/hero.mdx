---
title: Hero
---

import { LinkButton } from '@astrojs/starlight/components'

![Hero component](../../../assets/components/hero.png)

<LinkButton href="http://localhost:6006/?path=/story/apps-brand-hero--default" variant="secondary" icon="external">Storybook</LinkButton>

## Import

```js
import { Hero } from '@/brand/components/hero'
```

## Usage

```js
import { Button } from '@/ui/button'

<Hero
  title="Lorem ipsum dolor sit amet"
  description="Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris."
  cta={
    <>
      <Button asChild>
        <a href="/">Get started</a>
      </Button>
      <Button asChild variant="ghost">
        <a href="/">Learn more →</a>
      </Button>
    </>
  }
/>
```

