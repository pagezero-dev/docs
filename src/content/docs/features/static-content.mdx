...
- Frontmatter as JS exports — import assets, reference config, full TypeScript checking
- Zod-validated frontmatter — missing fields throw at load time, not silently
...
1. Add a detail route that loads a single item by slug:
   ...
   export const Route = createFileRoute("/_brand-layout/docs/$slug")({
     loader: ({ params }) => {
       ...
       return { doc: mod.frontmatter, DocComponent: mod.default }
     },
     component: DocPage,
   })

   function DocPage() {
     const { doc, DocComponent } = Route.useLoaderData()
     return (
       <MDXProvider>
         <DocComponent />
       </MDXProvider>
...
Frontmatter is a named JS export from the `.mdx` file rather than YAML. This allows importing local image assets processed by Vite, referencing the live config object, and getting TypeScript checking on every field.
...
- `crawlLinks: true` — the prerenderer follows `<a>` tags to discover all routes automatically
